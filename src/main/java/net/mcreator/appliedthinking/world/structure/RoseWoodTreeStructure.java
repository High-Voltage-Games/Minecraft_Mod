...
package net.mcreator.appliedthinking.world.structure;
...
public class RoseWoodTreeStructure extends AppliedThinkingModElements.ModElement {
	public RoseWoodTreeStructure(AppliedThinkingModElements instance) {
		super(instance, 253);
	}
	...
	public void init(FMLCommonSetupEvent event) {
				...
				if ((random.nextInt(1000000) + 1) <= 1000000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("applied_thinking", "rosewoodtree"));
						if (template == null)
							return false;
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("applied_thinking:rose_wood_bio")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
